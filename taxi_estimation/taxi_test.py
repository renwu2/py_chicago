...
@editor: Ren Wu
eidited on 10:53 2017/2/25
"""
# from tools import *
from routing.node import haversine_distance
...
class Trip:
    ...
    def __init__(self, record):
        ...
        [trip_id, taxi_id, trip_start_time, trip_end_time, trip_seconds, trip_miles, pickup_census, dropoff_census, pickup_community,
         dropoff_conmmunity, fare, tips, tolls, extra, toatal_fare, payment_type, company, pickup_latitude, pickup_longitude, pickup_point
         dropoff_latitude, dropoff_longitude, dropoff_point] = record
        ...
        trip_start_time = datetime.strptime(trip_start_time, '%m/%d/%y %H:%M').strftime('%Y,%m,%d,%H,%M')
        trip_end_time = datetime.strptime(trip_end_time, '%m/%d/%y %H:%M').strftime('%Y,%m,%d,%H,%M')
        try:
            [self.start_year, self.start_month, self.start_day, self.start_hour, self.start_minute] = map(int, re.findall(r'\d+', trip_start_time))
        except:
            [self.start_year, self.start_month, self.start_day, self.start_hour, self.start_minute] = [0]*5

        try:
            [self.end_year, self.end_month, self.end_day, self.end_hour, self.end_minute] = map(int, re.findall(r'\d+', trip_end_time))
        except:
            [self.start_year, self.start_month, self.start_day, self.start_hour, self.start_minute] = [0]*5


        #duration = self.dropoff_time - self.pickup_time  # Dropoff time is used to compute duration (timedelta object)
        #self.time = trip_seconds  # Time stores the duration as seconds
        ...
        self.straight_line_dist = haversine_distance(self.fromLat, self.fromLon, self.toLat, self.toLon)
